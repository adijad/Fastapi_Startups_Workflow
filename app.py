...
def scrape_startups(
    ...
    # very important: always keep a default safety ceiling
    ...
    while page <= effective_max_pages:
        ...
        html = get_page_html(page_url)
        html_hash = page_fingerprint(html)

        # stop if the exact page HTML repeats
        ...
        for card in cards:
            parsed = parse_startup_card(card)
            if parsed and is_relevant(parsed):
                page_startups.append(parsed)

        page_startups = dedupe_startups(page_startups)
        pages_scraped += 1
        ...
        # stop if the page has the same set of names as a previous page
        current_name_set = tuple(sorted(normalize(s["name"]) for s in page_startups if s.get("name")))
        ...
        # if page produced no new unique startups, pagination is effectively over
